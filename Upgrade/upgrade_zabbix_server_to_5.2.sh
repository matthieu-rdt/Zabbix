...
# description
# upgrade zabbix up to version number 5.2 for multiple distros (ubuntu, debian, rhel)
...
upgrade_ubuntu_debian ()
{
	...
	curl -O "https://repo.zabbix.com/zabbix/5.2/$OS/pool/main/z/zabbix-release/zabbix-release_5.2-1+$OS$(lsb_release -sr)_all.deb"
...
upgrade_rhel () 
{
	...
	sudo rpm -Uvh https://repo.zabbix.com/zabbix/5.2/rhel/$(rpm -E %{rhel})/x86_64/zabbix-release-5.2-1.el$(rpm -E %{rhel}).noarch.rpm
	sudo $pktm clean all
